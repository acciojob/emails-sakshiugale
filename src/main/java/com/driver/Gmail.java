...
public class Gmail extends Email {
    ...
    private LinkedList<Mail> trash;//Trash: Stores mails. Each mail has date (Date), sender (String), message (String)
    ...
    public void receiveMail(Date date, String sender, String message){
        if (inbox.size() >= inboxCapacity) {    ////If the inbox is full, move the oldest mail in the inbox to trash and add the new mail to inbox.
            trash.addFirst(inbox.removeLast()); // It is guaranteed that:
        }
        inbox.addFirst(new Mail(date, sender, message)); // 1. Each mail in the inbox is distinct.
    ...
    public int findMailsBetweenDates(Date start, Date end){
        ...
        for (Mail mail : inbox) {
            ...
            if (mailDate.equals(start) || mailDate.equals(end)) {
                count++;
            }
        }
        return count;
